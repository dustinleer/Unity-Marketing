		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
				...
				if ( isset( $woo_options['woo_font_page_title'] ) && $woo_options['woo_font_page_title'] )
					$output .= '.page #main header h1 { '.woo_generate_font_css($woo_options[ 'woo_font_page_title' ]).' }' . "\n";
...
add_action( 'woo_top', 'woo_top_nav' );
...
function woo_customise_testimonials_template ( $template ) {
	return '<div id="quote-%%ID%%" class="%%CLASS%%"><div class="quote-content">%%AVATAR%%<blockquote class="testimonials-text">%%TEXT%% %%AUTHOR%%</blockquote></div></div>';
