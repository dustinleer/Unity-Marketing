...
if ( ! function_exists( 'wooframework_feature_check' ) ) {
	...
	} // End wooframework_feature_check()
}
...
if ( ! function_exists( 'woocommerce_theme_before_content' ) ) {
	function woocommerce_theme_before_content() {
	    ...
	    <div id="content" class="col-full <?php if ( ! is_product() ) echo esc_attr( $columns ); ?>">
...
add_action( 'woocommerce_before_shop_loop_item_title', 'peddlar_product_defaults_wrap_open' , 20 ); //opener
add_action( 'woocommerce_after_shop_loop_item_title', 'peddlar_product_defaults_wrap_close', 40); //closer

function peddlar_product_defaults_wrap_open() {
	echo '<div class="product-details">';
}
function peddlar_product_defaults_wrap_close() {
...
// Header search form
add_action( 'woo_nav_after', 'woocommerce_search_widget', 30 );
...
if ( ! function_exists( 'woo_cart_link' ) ) {
	function woo_cart_link() {
		global $woocommerce;
		?>
			<a class="cart-contents" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'woothemes' ); ?>"><span class="price"><?php echo $woocommerce->cart->get_cart_total(); ?></span><span class="contents"><?php echo sprintf( _n('%d item', '%d items', $woocommerce->cart->get_cart_contents_count(), 'woothemes' ), $woocommerce->cart->get_cart_contents_count() );?></span></a>
		<?php
	}
}